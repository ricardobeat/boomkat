...
- [ ] **`max_heap_reg == 0` is both a sentinel and a valid register index** — `decref_callee_regs` (`src/vm/vm_core.c3:589`) early-returns on zero, so in principle a frame whose only heap register is register 0 would leak it. **Probed for a reachable shape and found none.** Two candidate shapes both hold flat RSS over 200-300k iterations against a control: a heap argument in r0 (`function f(a){ return 1; }`) is *borrowed*, not owned, so the callee has nothing to release (`vm_calls.c3:435`); a heap value written straight into r0 (`function f(){ return {}; }`) reaches the caller through `result_slot`, which raises the *caller's* watermark at `vm_core.c3:698`. So the sentinel collision is real in the code but currently unreachable, kept alive by two separate ownership rules. Left as-is: the fix (a one-based count) touches every use site, and changing it without a failing case risks more than it buys. Worth revisiting if either ownership rule changes
- [ ] **Bulk register restores must maintain `max_heap_reg` themselves** — the two generator/async resume paths are the complete set today (every other `saved_regs[i]` use is a save, not a restore), but any future path that bulk-copies values into a frame's registers has the same defect and the same signature. The stale comment that claimed the over-scan was safe because unused slots "are zero-initialised to undefined" has been corrected in place: the scan is sound only while every pop leaves its registers cleared, and narrowing the span does not help because the stale slot can fall inside `num_regs + 4` just as easily
