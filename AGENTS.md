...
- **test262 skip list**: ~60% of test262 falls outside this engine's scope, which is ES5/ES6 core plus the sloppy-mode and Annex B behavior it needs (ECMA-402, Stage 3 proposals, host-specific and cross-realm behavior stay out). Scope is documented in `docs/engine-scope.md`. The skip list (`SKIP_DIRS`/`SKIP_GLOBS`/`SKIP_FILES`/`UNSUPPORTED_PATTERN`) is embedded directly in `scripts/run_test262.py`: update it there when implementing new features. The skip list is the *only* place scope is expressed — test selection itself is exhaustive over test262's directory tree, so a feature is out of scope because a rule names it, never because nobody listed its directory. `intl402` (ECMA-402) is skipped per test262's own guidance; `annexB` is skipped wholesale (measured at 654/1086 passing, the rest blocked on the legacy eval-code and global-code var-hoisting rules and the Annex B String/Date methods); `staging` runs, as upstream `INTERPRETING.md` asks.
...
- The remaining `annexB` failures are not sloppy-mode syntax gates: the legacy eval-code and global-code var-hoisting rules (197 + 64 tests), the Annex B String HTML-wrapper and Date `getYear`/`setYear` methods, and `catch (x) { for (var x …) }` redeclaration.
